...
| 版本 | 内容 |
|---|---|
| **v2.6.0（当前）** | 军团永久化重构（军团旗组建、军团长不可更换+统帅50%、成员≤10人、攻城后全体驻守、可撤走成员）；军团旗（100万人口基准、难10倍）、加速符（训练/行军-1小时）；一键训练/一键结束；将领显示所属军团与军团长小红旗 |
...
- **一键训练/一键结束**：将领页批量开始/停止所有空闲与驻守将领的训练；将领卡片显示所属军团（【军团名】），军团长带 🚩 小红旗。
