...
| 版本 | 内容 |
|---|---|
| **v2.3.0（当前）** | 授权码登录系统（多存档隔离、管理员 ainiyiwannian 可新增授权码）、驻守将领可训练（停止后恢复驻守）、己方城市无路线可用神行符增援、驻守将领可从驻守地调兵攻打周边 |
...
- **训练**：无人数上限，时长随批次人数增长（600 秒基础 + 每人 0.6 秒，100 人约 11 分钟、1000 人约 20 分钟）；完成时 1/10,000 概率诞生将领；取消返还 50%。
- **合成**：步兵 = 训练人口 + 武器 + 盔甲；骑兵另 +1 战马；原子操作。
- **将领**：统帅 = (200 + (等级-1)×100) × (1 + 统帅之道)；训练 1 经验/秒（治军加成），升级需 300×等级²；战力加成 1 + 等级×0.02。**空闲或驻守中的将领均可训练**（训练期间保留驻守地，停止后恢复驻守）。
- **军团**：1 将领 + 步兵 + 骑兵；人数 ≤ 统帅上限；速度 = 步兵占比×1.0 + 骑兵占比×1.8（军驿加成）；海路 ×1.5。**驻守将领可从驻守地调兵攻打周边**（兵力取自驻军，支持神行符远征）；**己方城市无直达路线时可用神行符增援**（同省 1 张起）。
...
POST /api/armies/create | march | cancel-march | transfer   # 支持 useTalisman
