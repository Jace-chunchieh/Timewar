...
| 版本 | 内容 |
|---|---|
| **v2.5.0（当前）** | 训练将领概率降至 1/20,000；增援免将领、攻城必须将领；多将领出征（1~3 将，统帅叠加，副将减半加成）；战斗经验与将领负伤；战斗策略（稳守/突袭）与军团番号；将领天赋（5 种）；敌方反攻；蛮族营地；首都迁移（人口×1.5）；省域完成奖励（+10%）；统一全国通关结局 |
...
- **军团**：1~3 名将领（多将领出征：统帅 = Σ各将统帅，战力加成 = 主将全额 + 副将减半）；人数 ≤ 合计统帅上限；速度 = 步兵占比×1.0 + 骑兵占比×1.8（军驿加成）；海路 ×1.5。**增援（己方城市）无需将领，攻城必须将领**；战斗可选策略（稳守/突袭）；可自定义军团番号（≤8字）。
